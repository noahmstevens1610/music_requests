...
function AlbumArt({ image, name, size = "large" }: { image: string | null; name: string; size?: "small" | "large" }) {
  const classes = size === "large" ? "h-20 w-20 " : "h-[72px] w-[72px]  sm:h-[88px] sm:w-[88px]";
              ...
              >
                ...
                <div className="min-w-0">
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    {songRequest.explicit ? <ExplicitBadge /> : null}
                  </div>
                ...
                <div className="flex shrink-0 flex-col items-center">
                  ...
                  <button
                    ...
                    className={`font-heading mt-3 min-w-[72px]  border-2 px-3 py-2 text-lg uppercase tracking-[0.06em] transition duration-200 active:scale-95 sm:min-w-[86px] sm:text-xl ${
                      hasVoted
    ...
    <main className="relative min-h-screen overflow-hidden bg-black px-4 pt-8 pb-7 text-white sm:px-6 sm:pt-10 sm:pb-10">
      ...
      <div className="relative mx-auto max-w-6xl">
        ...
        <section className="mt-5 border border-white/10 bg-[#0d0d0d]/95 p-4 shadow-2xl sm:p-6">
            ...
            <div className="relative">
              ...
              {query && (
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2  px-3 py-1 text-2xl text-white/45 hover:bg-white/10 hover:text-white"
                >
                  ×
                </button>
              )}
          ...
          {message && <p className="mt-4  border border-white/10 bg-white/5 p-3 text-sm text-white/75">{message}</p>}
          {voteError && <p className="mt-4  border border-red-500/40 bg-red-950/50 p-3 text-sm text-red-200">{voteError}</p>}
        </section>
        ...
        <section className="mt-4 space-y-3">
          ...
          {searchResults.map((track) => (
            <button
              ...
              className="group flex w-full items-center gap-4  border border-white/10 bg-[#0d0d0d] p-3 text-left shadow-lg transition duration-200 hover:-translate-y-0.5 hover:border-[#c4202f]/70 hover:bg-[#151515] hover:shadow-xl"
            >
              ...
              <span className="font-heading  bg-white px-4 py-2 text-lg uppercase tracking-[0.05em] text-black">Select</span>
            </button>
          ))}
        </section>
      ...
      {selectedTrack && (
        <div className="fixed inset-0 z-50 flex animate-[fadeIn_180ms_ease-out] items-end justify-center bg-black/85 p-4 backdrop-blur-sm sm:items-center" onClick={closeRequestModal}>
          <div
            ...
            className="w-full max-w-lg animate-[scaleIn_180ms_ease-out]  border border-white/15 bg-[#0d0d0d] p-5 shadow-2xl sm:p-6"
          >
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  {selectedTrack.explicit ? <ExplicitBadge /> : null}
                </div>
                <p className="mt-1 truncate text-white/55">{selectedTrack.artist}</p>
              </div>
              <button type="button" onClick={closeRequestModal} className=" px-3 py-1 text-2xl text-white/50 hover:bg-white/10 hover:text-white">×</button>
            </div>
            ...
            <div className="mt-6">
              ...
              <div className="mt-3 grid grid-cols-2 gap-3">
                <button
                  ...
                  className={` border p-4 text-left transition ${requestType === "line_dance" ? "border-[#c4202f] bg-[#c4202f]/20" : "border-white/15 bg-black hover:border-white/30"}`}
                >
                ...
                <button
                  ...
                  className={` border p-4 text-left transition ${requestType === "swing" ? "border-white bg-white/10" : "border-white/15 bg-black hover:border-white/30"}`}
                >
            ...
            {modalError && <p className="mt-4  border border-red-500/40 bg-red-950/50 p-3 text-sm font-semibold text-red-200">{modalError}</p>}

            <button
              type="button"
              onClick={submitSong}
              disabled={!requestType || submitting}
              className="font-heading mt-5 w-full  bg-[#c4202f] px-5 py-4 text-2xl uppercase tracking-[0.05em] text-white transition hover:bg-[#d9293a] disabled:cursor-not-allowed disabled:opacity-40"
            >
